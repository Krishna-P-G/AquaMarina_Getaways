...
function Register_Password() {
  ...
  const handleSubmit = async (e) => {
    ...
    // Check if the phone number format is correct (+91 followed by 10 digits)
    if (!/^\d{10}$/.test(phone)) {
      errors.phoneFormat = "Please enter a valid 10-digit phone number.";
    }
    ...
    if (!allIconsCorrect && incorrectErrors.length > 0) {
      errors.passwordValidation = incorrectErrors[0];
    }
    ...
    const phone_no = phone.replace(/\D/g, "");
    ...
    try {
      ...
      Swal.fire({
        title: "Error!",
        text: "An error occurred while creating account.",
        icon: "error",
        confirmButtonText: "OK",
      });
    }
  ...
  return (
    <div className="w-full h-screen flex items-start">
      ...
      <div className="relative w-1/2 h-full flex flex-col">
        <img
          src="https://images.unsplash.com/photo-1495042823740-46feea76a69c?q=80&w=1922&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"
          alt="cover_image"
          ...
          <form onSubmit={handleSubmit}>
            <div className="w-full flex flex-col">
              <div className="relative">
                <input
                  type="tel"
                  placeholder="Phone Number (e.g., 7094985167)"
