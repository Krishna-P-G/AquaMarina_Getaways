...
import Register_States  from "./Register_States";
...
function Register_email() {
  ...
  const handleVerifyEmail = async (email) => {
    ...
    await Register_States.VerifyEmail(email.toLowerCase(), name)
      .then((res) => {
        setTimeout(() => {
          if (res.data === "Already Registered Email") {
            Swal.fire({
              title: "Error!",
              text: "Email is already registered, You will be redirected to login page.",
              icon: "error",
              confirmButtonText: "OK",
            }).then(() => {
              navigate("/login");
  ...
  const handleSubmit = async (e) => {
    ...
    navigate("/register/password", { state: { email, name } });
  };
