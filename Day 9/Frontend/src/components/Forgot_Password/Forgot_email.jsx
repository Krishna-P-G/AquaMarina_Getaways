...
import Forgot_States from "./Forgot_States";
...
function Forgot_email() {
  ...
  const handleVerifyEmail = async () => {
    ...
    await Forgot_States.SendForgotPassword(email.toLowerCase())
      .then((res) => {
        setTimeout(() => {
          if (res.data === "Email is not registered") {
            Swal.fire({
              title: "Error!",
              text: "Email is not registered, You will be redirected to signup page.",
              icon: "error",
              confirmButtonText: "OK",
            }).then(() => {
  ...
  useEffect(() => {
    // Execute side effect after state is updated
    if (generatedCode) {
      Swal.fire({
        title: "Success!",
        text: "Verification Code has been Sent to your Email",
